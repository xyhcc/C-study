...
![img](C语言学习笔记.assets/clip_image002.gif)

四个步骤并不是gcc独立完成的，而是在内部调用了其他工具，从而完成了整个工作流程，其中编译最耗时，因为要逐行检查语法。

![image-20220916155236559](C语言学习笔记.assets/image-20220916155236559-1666254787436.png)
...
<font color = red>注意：声明变量的语句必须以分号结尾，一旦声明，变量的类型就不能在运行时修改。</font>
...
<font color = red>任何长度需要运行时才能确定的数组，都是变长数组。</font>
...
<font color = pink>数组开空间的时候开2的倍数，如int a[32]  ==>  2、4、6、8、16、32、64、128等，用于内存对齐，提高空间利用率</font>

<font color = pink>数组名是个固定的地址，不能自加，不能赋值。</font>
...
<font color=skyblue>struct 结构占用的存储空间，不是各个属性存储空间的总和，而是最大内存占用属性的存储空间的倍数，其他属性会添加空位与之对齐。</font>这样可以提高读写效率。
...
```
https://blog.csdn.net/anyegongjuezjd/article/details/106891585
```
...
<font color = red>注意：只在当前文件里面使用的函数，也可以声明为`static`，表明该函数只在当前文件使用，其他文件可以定义同名函数：</font>
...
```
https://blog.csdn.net/weixin_43853307/article/details/108412943
```
